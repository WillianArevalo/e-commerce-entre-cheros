                    ...
                    @foreach ($orders as $order)
                        @php
                            ...
                            switch ($order->status) {
                                case 'pending':
                                    $styles =
                                        'bg-yellow-100 text-yellow-700 border-yellow-200 bg-opacity-70 shadow-yellow-200';
                                    break;
                                case 'sent':
                                    $styles = 'bg-blue-100 text-blue-700 border-blue-200 bg-opacity-70 shadow-blue-200';
                                    break;
                                case 'completed':
                                    $styles =
                                        'bg-green-100 text-green-700 border-green-200 bg-opacity-70 shadow-green-200';
                                    break;
                                case 'canceled':
                                    $styles = 'bg-red-100 text-red-700 border-red-200 bg-opacity-70 shadow-red-200';
                                    break;
                            }
                        @endphp
                        <div class="{{ $styles }} flex flex-col gap-2 overflow-hidden rounded-xl border p-4 shadow">
                            <div class="flex justify-between">
                                <h3 class="flex items-center gap-2 text-xl font-bold text-secondary">
                                    <x-icon-store icon="shopping-bag" class="h-5 w-5 text-current" />
                                    {{ $order->number_order }}
                                </h3>
                                @if ($order->status === 'pending')
                                    <span
                                        class="ml-auto flex items-center gap-1 rounded-xl border border-yellow-200 bg-yellow-400 px-2 py-1 text-xs font-medium text-white">
                                        <x-icon-store icon="clock" class="h-4 w-4 text-white" />
                                        {{ ucfirst($order->status) }}
                                    </span>
                                @elseif($order->status === 'sent')
                                    <span
                                        class="ml-auto flex items-center gap-1 rounded-xl border border-blue-200 bg-blue-500 px-2 py-1 text-xs font-medium text-white">
                                        <x-icon-store icon="truck-delivery" class="h-4 w-4 text-white" />
                                        {{ ucfirst($order->status) }}
                                    </span>
                                @elseif($order->status === 'completed')
                                    <span
                                        class="ml-auto flex items-center gap-1 rounded-xl border border-green-200 bg-green-400 px-2 py-1 text-xs font-medium text-white">
                                        <x-icon-store icon="checkmark-circle" class="h-4 w-4 text-white" />
                                        {{ ucfirst($order->status) }}
                                    </span>
                                @elseif($order->status === 'canceled')
                                    <span
                                        class="ml-auto flex items-center gap-1 rounded-xl border border-red-200 bg-red-500 px-2 py-1 text-xs font-medium text-white">
                            ...
                            <div class="flex justify-between">
                                ...
                                <div class="flex flex-col items-end gap-2">
                                    ...
                                    <span class="text-xs text-zinc-600">
                                        {{ $order->created_at->setTimeZone(auth()->timezone ?? 'UTC')->format('F y, Y, g:i A') }}
                                    </span>
                                </div>
                            </div>
        ...
        <div class="hidden" id="pending" role="tabpanel" aria-labelledby="pending-orders">
            @if ($ordersPending->count() > 0)
                ...
                <div class="mb-4 flex flex-col gap-4 lg:hidden">
                    @foreach ($ordersPending as $order)
                        <div
                            class="flex flex-col gap-2 overflow-hidden rounded-xl border border-yellow-200 bg-yellow-100 bg-opacity-70 p-4 text-yellow-700 shadow shadow-yellow-200">
                            <div class="flex justify-between">
                                <h3 class="flex items-center gap-2 text-xl font-bold text-secondary">
                                    <x-icon-store icon="shopping-bag" class="h-5 w-5 text-current" />
                                    {{ $order->number_order }}
                ...
                <div class="mb-4 flex flex-col gap-4 lg:hidden">
                    @foreach ($ordersApproved as $order)
                        @php
                            ...
                            if ($order->status === 'completed') {
                                $styles =
                                    'bg-green-100 text-green-700 border border-green-200 bg-opacity-70 shadow-green-200';
                            } else {
                                $styles =
                                    'bg-blue-100 text-blue-700 border border-blue-200 bg-opacity-70 shadow-blue-200';
                            }
                        @endphp
                        <div class="{{ $styles }} flex flex-col gap-2 overflow-hidden rounded-xl p-4 shadow">
                            <div class="flex justify-between">
                                <h3 class="flex items-center gap-2 text-xl font-bold text-secondary">
                                    <x-icon-store icon="shopping-bag" class="h-5 w-5 text-current" />
                                    {{ $order->number_order }}
        ...
        <div class="hidden" id="cancelled" role="tabpanel" aria-labelledby="cancelled-orders">
            @if ($ordersRejected->count() > 0)
                ...
                <div class="mb-4 flex flex-col gap-4 lg:hidden">
                    @foreach ($ordersRejected as $order)
                        <div
                            class="flex flex-col gap-2 overflow-hidden rounded-xl border border-red-200 bg-red-100 bg-opacity-70 p-4 text-red-700 shadow shadow-red-200">
                            <div class="flex justify-between">
                                <h3 class="flex items-center gap-2 text-xl font-bold text-secondary">
                                    <x-icon-store icon="shopping-bag" class="h-5 w-5 text-current" />
                                    {{ $order->number_order }}
                                ...
                                <span
                                    class="ml-auto flex items-center gap-1 rounded-xl border border-red-200 bg-red-400 px-2 py-1 text-xs font-medium text-white">
                                    <x-icon-store icon="checkmark-circle" class="h-4 w-4 text-white" />
                                    {{ ucfirst($order->status) }}
                                </span>
                            </div>
