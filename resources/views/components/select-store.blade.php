<div class="w-full">
    @if ($label != '')
        <label for="{{ $id }}"
            class="{{ $required ? "after:content-['*'] after:ml-0.5 after:text-red-500" : '' }} mb-2 block text-start text-sm text-zinc-600">
            {{ ucfirst($label) }}
        </label>
    @endif
    ...
    <div class="relative">
        <div
            class="selected @error($name) is-invalid @enderror flex w-full items-center justify-between rounded-xl border border-zinc-300 bg-zinc-50 px-6 py-3 text-sm">
        ...
        <ul
            ...
            @foreach ($options as $value => $label)
                <li class="itemOption truncate rounded-xl px-4 py-2.5 text-sm text-zinc-900 hover:bg-zinc-100"
